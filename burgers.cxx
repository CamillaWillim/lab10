...
int main(){
  ...
  for(int i=1; i<=Na; i++)
  {
   for(int j=0; j<Nk; j++){
     ...
     h = u0; u0 = u1; u1 = u2; u2 = h;// step + swap here
...
u2[N-1] = u0[N-1] - u1[N-1] * dt/dx * (u1[0] - u1[N-2]); // periodic boundary condition
u2[0] = u0[0] - u1[0] * dt/dx * (u1[1] - u1[N-1]); // periodic boundary condition
                ...
                const double dt, const double xmin,  const int N)
{
   ...
   {
     double x = xmin + i*dx;
     u = sin(2*M_PI*x); ux = 2*M_PI*cos(2*M_PI*x); uxx = -4*pow(M_PI,2)*sin(2*M_PI);
     u0[i] = u + dt* u* ux + pow(dt,2)* (u* pow(ux,2) + 0.5* pow(u,2) * uxx);
     u1[i] = u;
